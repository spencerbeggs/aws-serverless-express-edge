...
console.log(awsServerlessExpress);

test("getPathWithQueryStringParams: no params", () => {
  const event = {
    path: "/foo/bar"
  };
  const pathWithQueryStringParams = awsServerlessExpress.getPathWithQueryStringParams(
    event
  );
...
function mapApiGatewayEventToHttpRequest(headers) {
  ...
  const httpRequest = awsServerlessExpress.mapApiGatewayEventToHttpRequest(
    event,
    context,
    socketPath
  );
