...
test("defaults", () => {
  ...
  expect(req.apiGateway.event).toEqual(
    JSON.parse(decodeURIComponent(originalHeaders["x-apigateway-event"]))
  );
  expect(req.apiGateway.context).toEqual(
    JSON.parse(decodeURIComponent(originalHeaders["x-apigateway-context"]))
  );
...
test("options.reqPropKey", () => {
  ...
  expect(req._apiGateway.event).toEqual(
    JSON.parse(decodeURIComponent(originalHeaders["x-apigateway-event"]))
  );
  expect(req._apiGateway.context).toEqual(
    JSON.parse(decodeURIComponent(originalHeaders["x-apigateway-context"]))
  );
  expect(req.headers["x-apigateway-event"]).toBe(undefined);
  expect(req.headers["x-apigateway-context"]).toBe(undefined);
...
test("options.deleteHeaders = false", () => {
  ...
  expect(req.headers["x-apigateway-event"]).toEqual(
    originalHeaders["x-apigateway-event"]
  );
  expect(req.headers["x-apigateway-context"]).toEqual(
    originalHeaders["x-apigateway-context"]
  );
});
